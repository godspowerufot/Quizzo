...
/*When the action type is 'quiz', it clones the quiz data from action.value using _.cloneDeep to ensure a deep copy.
It then iterates through each question and option in the quiz and sets option.checked to false. This is likely used for initializing the quiz state, ensuring that all options are unchecked.
The updated qnaSet is returned as the new state.
*/
...
This reducer is likely used in conjunction with the useReducer hook in a React component. It would be used to manage the state of the quiz, such as keeping track of user responses and updating the UI accordingly.*/
    dispatch({
      type: 'quiz',
      value: quiz
    });
...
//
    ...
    try {
      ...
      // Navigates to the result page for the current quiz (id) and passes along additional data (the question and answer set, and the mark sheet).
